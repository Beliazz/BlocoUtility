...
// Basic types.
// --------------------------------------------------------------------------
typedef unsigned long  BLOCO_UTILITY_API dword;
typedef unsigned short BLOCO_UTILITY_API word;
typedef unsigned char  BLOCO_UTILITY_API byte;
...
// --------------------------------------------------------------------------
// --------------------------------------------------------------------------
struct BLOCO_UTILITY_API ZipFile::TZipLocalHeader
{
  enum
  {
...
// --------------------------------------------------------------------------
// --------------------------------------------------------------------------
struct BLOCO_UTILITY_API ZipFile::TZipDirHeader
{
  enum
  {
...
// --------------------------------------------------------------------------
// --------------------------------------------------------------------------
struct BLOCO_UTILITY_API ZipFile::TZipDirFileHeader
{
  enum
  {
...
// --------------------------------------------------------------------------
bool BLOCO_UTILITY_API ZipFile::Init(const _TCHAR *resFileName)
{
  End();
  ...
  m_pDirData = DEBUG_CLIENTBLOCK char[dh.dirSize + dh.nDirEntries*sizeof(*m_papDir)];
  if (!m_pDirData)
    return false;
...
optional<int> BLOCO_UTILITY_API ZipFile::Find(const char *path) const
{
...
// Parameters:    
// --------------------------------------------------------------------------
void BLOCO_UTILITY_API ZipFile::End()
{
...
// --------------------------------------------------------------------------
void BLOCO_UTILITY_API ZipFile::GetFilename(int i, char *pszDest)  const
{
  if (pszDest != NULL)
  {
    if (i < 0 || i >= m_nEntries)
...
// --------------------------------------------------------------------------
int BLOCO_UTILITY_API ZipFile::GetFileLen(int i) const
{
  if (i < 0 || i >= m_nEntries)
    return -1;
  else
...
// --------------------------------------------------------------------------
bool BLOCO_UTILITY_API ZipFile::ReadFile(int i, void *pBuf)
{
  ...
  char *pcData = DEBUG_CLIENTBLOCK char[h.cSize];
  if (!pcData)
    return false;
...
// Parameters:    The file index and the pre-allocated buffer
// --------------------------------------------------------------------------
bool BLOCO_UTILITY_API ZipFile::ReadLargeFile(int i, void *pBuf, void (*callback)(int, bool &))
    ...
    if (f)
    {
      ...
      else
      {
        for (int i = 0; i < zip.GetNumFiles(); i++)
        {
          ...
          char *pData = DEBUG_CLIENTBLOCK char[len];
          if (!pData)
